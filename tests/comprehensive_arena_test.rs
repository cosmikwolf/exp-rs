...
use bumpalo::Bump;
use exp_rs::{EvalContext, expression::ArenaBatchBuilder, Real};
use std::rc::Rc;
...
fn test_batch_zero_allocations_with_functions() {
    ...
    #[cfg(not(feature = "libm"))]
    {
        ...
        ctx.register_native_function("sin", 1, |args| sin(args[0] as f64) as Real).unwrap();
        ctx.register_native_function("cos", 1, |args| cos(args[0] as f64) as Real).unwrap();
        ctx.register_native_function("sqrt", 1, |args| sqrt(args[0] as f64) as Real).unwrap();
    }
    
    // Also test expression functions
    ctx.register_expression_function("double", &["x"], "x * 2").unwrap();
    ctx.register_expression_function("pythag", &["a", "b"], "sqrt(a*a + b*b)").unwrap();
    ...
    builder.add_expression("sin(x) * cos(y) + sqrt(x*x + y*y)").unwrap();
    ...
    println!("Arena after first eval: {} bytes (growth: {})", 
             bytes_after_first, bytes_after_first - bytes_after_setup);
    ...
    for i in 0..100 {
        ...
        assert_eq!(current_bytes, bytes_after_first,
    ...
    println!("✓ Zero arena growth during 100 batch evaluations with native and expression functions!");
}
...
fn test_expression_api_zero_allocations() {
    ...
    for i in 0..100 {
        ...
        assert_eq!(arena.allocated_bytes(), bytes_after_first,
                   "Arena grew during Expression API evaluation #{}", i);
    }

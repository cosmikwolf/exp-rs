...
fn test_equal_operator() {
    ...
    #[cfg(feature = "f32")]
    assert_eq!(
        interp("0.1 + 0.2 == 0.3", Some(ctx_rc)).unwrap(),
        1.0,
...
fn test_not_equal_operator() {
    ...
    #[cfg(feature = "f32")]
    assert_eq!(
        interp("0.1 + 0.2 != 0.3", Some(ctx_rc.clone())).unwrap(),
        0.0,
...
fn test_comparison_with_expressions() {
    ...
    assert_eq!(
        interp("(2 * 3) != (5 + 2)", Some(ctx_rc.clone())).unwrap(),
        1.0
    );
    assert_eq!(
        interp("(3 * 4) > (10 + 1)", Some(ctx_rc.clone())).unwrap(),
        1.0
    );
    assert_eq!(
        interp("(5 - 2) < (10 / 2)", Some(ctx_rc.clone())).unwrap(),
        1.0
    );
    assert_eq!(
        interp("(10 / 2) >= (6 - 1)", Some(ctx_rc.clone())).unwrap(),
        1.0
    );
    assert_eq!(
        interp("(4 ^ 2) <= (4 * 4)", Some(ctx_rc.clone())).unwrap(),
        1.0
    );
}
...
fn test_comparison_with_variables() {
    ...
    assert_eq!(
        interp("(x + y) > (x - y)", Some(ctx_rc.clone())).unwrap(),
        1.0
    );
...
fn test_registered_comparison_function() {
    ...
    ctx.register_expression_function("is_positive", &["x"], "x > 0")
        .unwrap();
    ...
    assert_eq!(
        interp("is_positive(-5)", Some(ctx_rc.clone())).unwrap(),
        0.0
    );

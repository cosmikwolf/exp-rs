...
mod unit {
    ...
    fn test_parse_decimal_with_leading_dot() {
        ...
        let result2 = interp("sin(.5) + cos(.5)", None).unwrap();
        let expected = sin(0.5, 0.0) + cos(0.5, 0.0);
        #[cfg(feature = "f32")]
    ...
    fn test_array_access() {
        ...
        ctx.arrays
            .insert("climb_wave_wait_time".to_string().into(), vec![10.0, 20.0, 30.0]);
        // Use the new API to parse and evaluate the array access expression
        let val = interp("climb_wave_wait_time[1]", Some(std::rc::Rc::new(ctx.clone()))).unwrap();
        assert_eq!(val, 20.0);
    }

...
fn test_expression_function_with_arena() {
    ...
    assert_eq!(
        builder.add_expression("double(x) + polynomial(y)").unwrap(),
        2
    );
...
fn test_nested_expression_functions() {
    ...
    let mut ctx = EvalContext::new();
    ctx.register_expression_function("add_one", &["x"], "x + 1")
        .unwrap();
    ctx.register_expression_function("double", &["x"], "x * 2")
        .unwrap();
    ctx.register_expression_function("compose", &["x"], "double(add_one(x))")
        .unwrap();
...
fn test_expression_function_zero_allocations() {
    ...
    // Create context
    let mut ctx = EvalContext::new();
    ctx.register_expression_function("compute", &["x", "y"], "x^2 + y^2 + sin(x*y)")
        .unwrap();
...
fn test_expression_function_with_all_param_types() {
    ...
    ctx.register_expression_function("zero_params", &[], "42")
        .unwrap();
    ctx.register_expression_function("one_param", &["x"], "x * x")
        .unwrap();
    ctx.register_expression_function("three_params", &["a", "b", "c"], "a + b * c")
        .unwrap();
...
fn test_syntax_validation_at_registration() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        exp_rs::error::ExprError::UnmatchedParenthesis { .. }
    ));
    ...
    assert!(matches!(
        result.unwrap_err(),
        exp_rs::error::ExprError::Syntax(_)
    ));
...
fn test_semantic_validation_deferred_to_evaluation() {
    ...
    ctx.register_expression_function("uses_undefined", &["x"], "undefined_func(x)")
        .unwrap();

    // 2. Wrong function arity - registers OK
    ctx.register_native_function("sin", 1, |args| args[0].sin())
        .unwrap();
    ctx.register_expression_function("bad_sin", &["x"], "sin(x, 2, 3)")
        .unwrap();

    // 3. Undefined variable reference - registers OK
    ctx.register_expression_function("uses_undefined_var", &["x"], "x + undefined_var")
        .unwrap();

    // 4. Function used as variable - registers OK
    ctx.register_expression_function("func_as_var", &["x"], "sin + x")
        .unwrap();
    ...
    assert!(matches!(
        result.unwrap_err(),
        exp_rs::error::ExprError::UnknownFunction { .. }
    ));
    ...
    assert!(matches!(
        result.unwrap_err(),
        exp_rs::error::ExprError::InvalidFunctionCall { .. }
    ));
    ...
    assert!(matches!(
        result.unwrap_err(),
        exp_rs::error::ExprError::UnknownVariable { .. }
    ));
...
fn test_parameter_validation() {
    ...
    let many_params: Vec<&str> = (0..20)
        .map(|i| Box::leak(format!("p{}", i).into_boxed_str()) as &str)
        .collect();
...
fn test_function_name_validation() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        exp_rs::error::ExprError::StringTooLong
    ));
...
fn test_expression_function_capacity() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        exp_rs::error::ExprError::CapacityExceeded(_)
    ));
}
...
fn test_recursive_expression_functions() {
    ...
    ctx.register_expression_function("factorial", &["n"], "n * factorial(n-1)")
        .unwrap();

    // Mutual recursion - both register OK
    ctx.register_expression_function("even", &["n"], "n == 0 || odd(n-1)")
        .unwrap();
    ctx.register_expression_function("odd", &["n"], "n != 0 && even(n-1)")
        .unwrap();
...
fn test_expression_with_context_variables() {
    ...
    ctx.register_expression_function("scale", &["x"], "x * global_scale")
        .unwrap();
...
fn test_unused_parameters_in_expression() {
    ...
    ctx.register_expression_function("ignores_y", &["x", "y"], "x * 2")
        .unwrap();
...
fn test_validated_registration_syntax_errors() {
    ...
    let report = ctx
        .register_expression_function_validated("bad_syntax", &["x"], "x + (2 * 3", true)
        .unwrap();
...
fn test_validated_registration_undefined_functions() {
    ...
    let report = ctx
        .register_expression_function_validated(
            "uses_undefined",
            &["x"],
    ...
    assert!(
        report
            .undefined_functions
            .contains(&"undefined_func".to_string())
    );
    assert!(
        report
            .undefined_functions
            .contains(&"another_undefined".to_string())
    );
...
fn test_validated_registration_arity_warnings() {
    ...
    let report = ctx
        .register_expression_function_validated("bad_arity", &["x"], "sin(x, 2, 3) + pow(x)", true)
        .unwrap();
...
fn test_validated_registration_undefined_variables() {
    ...
    assert!(
        report
            .undefined_variables
            .contains(&"undefined_var".to_string())
    );
    assert!(
        report
            .undefined_variables
...
fn test_validated_registration_all_valid() {
    ...
    let report = ctx
        .register_expression_function_validated("compute", &["x", "y"], "sin(x) * y + scale", true)
        .unwrap();
...
fn test_validated_registration_complex_expression() {
    ...
    ctx.register_expression_function("helper", &["a"], "a * 2")
        .unwrap();

...
fn test_fibonacci_recursion() {
    ...
    ctx.register_expression_function("fib", &["n"], "n <= 1 ? n : fib(n - 1) + fib(n - 2)")
        .unwrap();
...
fn test_mutual_recursion() {
    ...
    ctx.register_expression_function("is_even", &["n"], "n == 0 ? 1 : is_odd(n - 1)")
        .unwrap();

    ctx.register_expression_function("is_odd", &["n"], "n == 0 ? 0 : is_even(n - 1)")
        .unwrap();
...
fn test_recursion_limits() {
    ...
    println!(
        "Linear recursion limit: factorial({}) succeeds",
        linear_limit
    );
    ...
    assert!(
        exp_limit < linear_limit,
        "Exponential recursion should hit limit sooner"
    );
}

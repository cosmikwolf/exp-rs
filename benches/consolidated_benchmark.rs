...
fn measure_stage<F: FnOnce() -> R, R>(name: &str, f: F) -> (R, AllocationStats) {
    ...
    println!("{:<40} {:>10} bytes in {:>6} allocations", 
             name, 
...
fn calculate_ast_size(ast: &AstExpr) -> usize {
    ...
    match ast {
        ...
        AstExpr::Array { name, index } => {
            base_size + name.len() + calculate_ast_size(index)
        }
        AstExpr::Attribute { base, attr } => {
            base_size + base.len() + attr.len()
        }
        ...
        AstExpr::Conditional { condition, true_branch, false_branch } => {
            base_size + calculate_ast_size(condition) + 
            calculate_ast_size(true_branch) + calculate_ast_size(false_branch)
        }
    }
}
...
fn bench_expression_complexity(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("complex", "exp(a/10) * log(b+1) + pow(c, 0.5) * d + min(e, max(f, g))"),
        ("real_world", "a*sin(b*3.14159/180) + c*cos(d*3.14159/180) + sqrt(e*e + f*f)"),
    ];
...
fn run_memory_analysis() {
    ...
    let (ctx, _) = measure_stage("Creating context", || {
        create_test_context()
    });
    ...
    let (_, stats) = measure_stage("Subsequent 999 evaluations", || {
        for iteration in 1..1000 {
            let mut ctx_clone = (*ctx).clone();
            for (i, name) in param_names.iter().enumerate() {
                ctx_clone.set_parameter(name, (i + iteration) as f64).unwrap();
            }
    ...
    println!("\nPer-evaluation cost after first: ~{} bytes", per_eval_bytes);
    ...
    let (_, stats) = measure_stage("Subsequent 999 batch evaluations", || {
        for iteration in 1..1000 {
            for (i, name) in param_names.iter().enumerate() {
                builder.set_param_by_name(name, (i + iteration) as f64).unwrap();
            }
            builder.eval(&ctx).unwrap();
        }
    });
    ...
    println!("\nPer-evaluation cost after first: ~{} bytes", batch_per_eval_bytes);
...
fn run_ast_size_analysis() {
    ...
    let expressions = vec![
        ("simple", "a + b"),
        ("medium", "sin(a) * cos(b) + sqrt(c*c + d*d)"),
        ("complex", "a*sin(b*3.14159/180) + c*cos(d*3.14159/180) + sqrt(e*e + f*f)"),
        ("conditional", "a > 5 ? b * 2 : c / 3"),
        ("logical", "((a > 5) && (b < 10)) * c + ((d >= e) || (f != g)) * h"),
    ];
    ...
    println!("Memory traffic per second: {} MB", bytes_per_second / 1_000_000);
    println!("Memory traffic per minute: {} MB", bytes_per_second * 60 / 1_000_000);
    println!("Memory traffic per hour: {} GB", bytes_per_second as f64 * 3600.0 / 1_000_000_000.0);
}
...
fn run_cpu_utilization_test() {
    ...
    let individual_us_per_eval = (individual_duration.as_micros() as f64) / (iterations as f64 * 7.0); // 7 expressions
    ...
    println!("BatchBuilder approach: {:.0} Hz ({:.1}% of target)", batch_rate, (batch_rate / 1000.0) * 100.0);
    ...
    println!("  BatchBuilder uses: {:.1} µs ({:.1}% of budget)", batch_us_per_iter, (batch_us_per_iter / 1000.0) * 100.0);
    ...
    println!("  Cache misses likely: {:.0}/second", individual_rate * 7.0 * 10.0); // Rough estimate
    ...
    println!("  Expected CPU usage at 1000Hz: {:.1}%", (projected_us / 1000.0) * 100.0);
}
...
criterion_group!(benches, bench_comparison, bench_expression_complexity, bench_memory_analysis);

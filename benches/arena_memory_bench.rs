...
fn main() {
    ...
    println!("\nResults:");
    println!("- Time for 1000 evaluations: {:.2} ms", duration.as_millis());
    println!("- Arena bytes after 1000 evals: {} KB", bytes_after_eval / 1024);
    println!("- Additional bytes allocated: {} bytes", bytes_after_eval - bytes_after_setup);
    println!("- CPU usage at 1000Hz: {:.1}%", (duration.as_secs_f64() * 100.0));
    
    println!("\n=== Memory Savings ===");
    println!("Without arena: ~9,550 bytes × 1000 = 9.55 MB allocated/freed");
    println!("With arena: {} bytes allocated", bytes_after_eval - bytes_after_setup);

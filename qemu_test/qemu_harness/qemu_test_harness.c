...
void init_hardware_timer(void) {
    ...
    *TIMER1_CONTROL = control | TIMER_CTRL_ENABLE;
    ...
    if (start > end) {
        ...
        qemu_print("First timer address didn't work, trying alternative address...\n");
        ...
        *TIMER1_CONTROL = control | TIMER_CTRL_ENABLE;
        ...
        if (start > end) {
            qemu_printf("Alternative CMSDK Timer test: start=%u, end=%u, diff=%u ticks\n", 
                      start, end, start - end);
...
void benchmark_start(void) {
    // Initialize timer if not done already
...
void check_counter_rollover(void) {
    ...
    if ((*TIMER1_CONTROL & TIMER_CTRL_ENABLE) == 0) {
        qemu_printf("WARNING: CMSDK Timer disabled during measurement. Re-enabling...\n");
        
        uint32_t control = *TIMER1_CONTROL & ~TIMER_CTRL_ENABLE; // Get current config without enable bit
    ...
    if (current >= prev_check) {
        ...
        qemu_printf("WARNING: CMSDK Timer not counting down properly: prev=%u, current=%u\n",
                   prev_check, current);
        ...
        *TIMER1_CONTROL = TIMER_CTRL_32BIT | TIMER_CTRL_PERIODIC | TIMER_CTRL_ENABLE;
...
uint32_t benchmark_stop(void) {
    ...
    // Since the timer counts down, the start value should be larger than the end value
...
int qemu_file_open(const char *filename, const char *mode) {
  ...
  int mode_val = 0;
  if (strchr(mode, 'r') && !strchr(mode, '+')) mode_val = 0; // read
  if (strchr(mode, 'r') && strchr(mode, '+')) mode_val = 2; // read/write
  if (strchr(mode, 'w')) mode_val = 4; // write/create
  if (strchr(mode, 'a')) mode_val = 8; // append
  ...
  __asm__ volatile(
    ...
    "mov %0, r0\n"         // Get result
    : "=r" (fd)
    : "r" ((void *[]){(void *)filename, (void *)(uintptr_t)mode_val, (void *)strlen(filename)})
    : "r0", "r1", "memory"
  );

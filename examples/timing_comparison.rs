...
fn main() {
    ...
    for i in 0..1000 {
        for (p, &idx) in param_indices.iter().enumerate() {
            builder.set_param(idx, (p + 1) as f64 * 1.5 + i as f64 * 0.001).unwrap();
        }
        builder.eval(&ctx).unwrap();
    }
    ...
    println!("  Breakdown:");
    println!("    Parameter updates: {:.3} µs ({:.1}%)", 
             param_update_us, (param_update_us / full_cycle_us) * 100.0);
    println!("    Evaluation: {:.3} µs ({:.1}%)", 
             eval_only_us, (eval_only_us / full_cycle_us) * 100.0);
}
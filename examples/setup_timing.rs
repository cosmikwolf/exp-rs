use exp_rs::{BatchBuilder, EvalContext};
use std::rc::Rc;
...
fn main() {
    ...
    let start = Instant::now();
    for _ in 0..iterations {
        let _builder = BatchBuilder::new(&arena);
    }
    ...
    for _ in 0..100 { // Fewer iterations due to arena reset
        let arena = Bump::new();
    ...
    println!("   Context creation:    {:>6.1} µs ({:>4.1}%)", ctx_us, (ctx_us / total_us) * 100.0);
    println!("   Arena creation:      {:>6.1} µs ({:>4.1}%)", arena_us, (arena_us / total_us) * 100.0);
    println!("   Builder creation:    {:>6.1} µs ({:>4.1}%)", builder_us, (builder_us / total_us) * 100.0);
    println!("   Add parameters:      {:>6.1} µs ({:>4.1}%)", param_us, (param_us / total_us) * 100.0);
    println!("   Parse expressions:   {:>6.1} µs ({:>4.1}%)", expr_us, (expr_us / total_us) * 100.0);
    let first_eval_us = total_us - (ctx_us + arena_us + builder_us + param_us + expr_us);
    println!("   First evaluation:    {:>6.1} µs ({:>4.1}%)", first_eval_us, (first_eval_us / total_us) * 100.0);
    println!("   ─────────────────────────────────────────");
    ...
    println!();
    println!("   Break-even point: {} evaluations", (total_us / 13.7) as usize + 1);
    println!("   At 1000 Hz for 1 second:");
    println!("     Setup overhead: {:.2}%", (total_us / 1_000_000.0) * 100.0);
    println!("     Amortized setup cost: {:.3} µs per evaluation", total_us / 1000.0);
}
...
if [ "$FLOAT_MODE" = "f32" ]; then
	meson setup "$BUILD_DIR" --cross-file=qemu_test/qemu_harness/arm-cortex-m7-qemu.ini -Denable_f64=false
else
	meson setup "$BUILD_DIR" --cross-file=qemu_test/qemu_harness/arm-cortex-m7-qemu.ini -Denable_f64=true
fi
